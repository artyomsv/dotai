...
- Update docs in the **same commit** as the feature change, and **push them with it**. Not a
  follow-up commit, not "before the session ends". If the docs are not ready, the change is not ready.
- **A stale metric is worse than no metric.** These files quote concrete figures — line counts, prop
  counts, field lists, response shapes, "N classes". A stale one actively misleads, because it reads
  as headroom or coverage the change just spent. If your change touches a file whose metric a
  CLAUDE.md quotes, update the metric in the same commit, even when that is the only thing that moved.
- **Review-driven fixes carry their docs too.** When a reviewer's finding changes behaviour, the
  affected docs go in the commit that fixes it — the reasoning is most accurate while the finding is
  fresh, and a PR description is not a substitute for a doc.
- **Verify before claiming.** `git status` clean of doc changes and the branch pushed, before you
  report the work complete.
